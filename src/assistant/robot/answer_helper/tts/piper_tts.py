...
if platform.system() == "Windows":
    PIPER_PATH = os.path.join("..", PIPER_DIR + "_windows", "piper.exe")
else:
    PIPER_PATH = os.path.join("..", PIPER_DIR + "_linux", "piper")
...
class PIPER_TTS(TTS):
    ...
    def _speak_internal(self) -> None:
        try:
            ...
            threading.Thread(
                target=self.remove_wav_file, args=(wav_path,), daemon=True
            ).start()
    ...
    def is_done(self) -> bool:
        return self.piper_state == PiperState.IDLE and (
